"""Pytest fixtures for the customer segmentation project."""

from __future__ import annotations

from pathlib import Path
from typing import Generator

import numpy as np
import pytest
from fastapi.testclient import TestClient

import train


@pytest.fixture
def sample_data() -> np.ndarray:
    """Generate stable sample data for unit tests."""
    return np.array(
        [
            [25, 50000, 45],
            [30, 60000, 50],
            [35, 70000, 55],
            [40, 80000, 60],
            [45, 90000, 65],
            [22, 45000, 40],
            [28, 55000, 48],
            [32, 65000, 52],
            [38, 75000, 58],
            [42, 85000, 62],
            [50, 100000, 70],
            [55, 110000, 75],
            [60, 120000, 80],
            [65, 130000, 85],
            [70, 140000, 90],
            [23, 48000, 42],
            [27, 52000, 46],
            [31, 62000, 51],
            [36, 72000, 56],
            [41, 82000, 61],
            [19, 40000, 35],
            [21, 42000, 38],
            [24, 46000, 41],
            [26, 50000, 44],
            [29, 58000, 49],
            [33, 68000, 53],
            [37, 74000, 57],
            [43, 86000, 63],
            [47, 94000, 67],
            [52, 104000, 72],
            [20, 41000, 36],
            [34, 69000, 54],
            [39, 79000, 59],
            [44, 89000, 64],
            [49, 99000, 69],
            [54, 109000, 74],
            [59, 119000, 79],
            [64, 129000, 84],
            [69, 139000, 89],
            [18, 38000, 33],
            [48, 96000, 68],
            [53, 106000, 73],
            [58, 116000, 78],
            [63, 126000, 83],
            [68, 136000, 88],
            [25, 51000, 45],
            [30, 61000, 50],
            [35, 71000, 55],
            [40, 81000, 60],
            [45, 91000, 65],
        ],
        dtype=np.float64,
    )


@pytest.fixture
def sample_csv(tmp_path: Path, sample_data: np.ndarray) -> Path:
    """Create a temporary CSV file with test data."""
    csv_path = tmp_path / "test_customers.csv"
    lines = ["age,annual_income,spending_score"]
    lines.extend(
        f"{int(row[0])},{int(row[1])},{int(row[2])}"
        for row in sample_data
    )
    csv_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return csv_path


@pytest.fixture
def invalid_csv(tmp_path: Path) -> Path:
    """Create a CSV with invalid data."""
    csv_path = tmp_path / "invalid.csv"
    csv_path.write_text(
        "age,annual_income,spending_score\n25,50000,abc\n",
        encoding="utf-8",
    )
    return csv_path


@pytest.fixture
def missing_columns_csv(tmp_path: Path) -> Path:
    """Create a CSV missing a required column."""
    csv_path = tmp_path / "missing_cols.csv"
    csv_path.write_text("age,annual_income\n25,50000\n", encoding="utf-8")
    return csv_path


@pytest.fixture
def trained_artifacts(tmp_path: Path, sample_csv: Path) -> Path:
    """Create real training artifacts for API tests."""
    artifacts_dir = tmp_path / "artifacts"
    train.run_training(
        input_path=sample_csv,
        artifacts_dir=artifacts_dir,
        n_clusters=5,
        random_state=42,
        stability_runs=3,
    )
    return artifacts_dir


@pytest.fixture
def api_client(trained_artifacts: Path, monkeypatch: pytest.MonkeyPatch) -> Generator[TestClient, None, None]:
    """Create a FastAPI client backed by trained test artifacts."""
    from api import main

    monkeypatch.setattr(main, "ARTIFACTS_DIR", trained_artifacts)
    monkeypatch.setattr(main, "KMEANS_PATH", trained_artifacts / "kmeans_model.pkl")
    monkeypatch.setattr(main, "SCALER_PATH", trained_artifacts / "scaler.pkl")
    monkeypatch.setattr(main, "FEATURE_ENGINEER_PATH", trained_artifacts / "feature_engineer.pkl")
    monkeypatch.setattr(main, "SEGMENT_CATALOG_PATH", trained_artifacts / "segment_catalog.json")
    monkeypatch.setattr(main, "METADATA_PATH", trained_artifacts / "model_metadata.json")

    main.model_container = main.ModelContainer()
    main.model_container.load_models()

    client = TestClient(main.app)
    yield client
    main.model_container = main.ModelContainer()
